...
class plgSystemModalsHelper
{
	function __construct(&$params)
	{
		...
		$this->params->mediafiles = explode(',', strtolower($this->params->mediafiles));
		$this->params->iframefiles = explode(',', strtolower($this->params->iframefiles));
		...
		)
		{
			...
			else
			{
				...
				if ($this->params->load_jquery)
				{
					...
					{
						if ($this->params->load_jquery === 2)
						{
							JHtml::script('modals/jquery-2.min.js', false, true);
						}
						else
						{
	...
	function replace(&$str, $area = '')
	{
		...
		if (preg_match_all($this->params->regex_inlink, $str, $matches, PREG_SET_ORDER) > 0)
		{
			foreach ($matches as $match)
			{
				...
				$link = $this->getLink($data, $match['1'], $content);
				$html = $link . '</a>';
		...
		if (preg_match_all($this->params->regex, $str, $matches, PREG_SET_ORDER) > 0)
		{
			foreach ($matches as $match)
			{
				...
				$link = $this->getLink($match['2'], '', trim($match['4']));
	...
	function getLink($str, $link = '', $content = '')
	{
		$html = '';
		...
		return $html . $this->buildLink($attribs, $data, $content);
	}
	...
	function buildLink($l, $data, $content = '')
	{
		$html = array();
